...
class function TNetCrossRouter.Index(const ALocalDir, AFileParamName: string; const ADefIndexFiles: TArray<string>): TCrossHttpRouterProc;
var
...
begin
  ...
  Result :=
    ...
    begin
      var LPath := ALocalDir;
      var LFile := ARequest.Params[AFileParamName];

      if (LFile = '') then
      begin
        for var LDefMainFile in LDefIndexFiles do
        begin
...
class function TNetCrossRouter.Static(const ALocalDir, AFileParamName: string): TCrossHttpRouterProc;
begin
  Result :=
    procedure(const ARequest: ICrossHttpRequest; const AResponse: ICrossHttpResponse; var AHandled: Boolean)
    var
...
class function TNetCrossRouter.Dir(const APath, ALocalDir, ADirParamName: string): TCrossHttpRouterProc;
begin
  Result :=
    procedure(const ARequest: ICrossHttpRequest; const AResponse: ICrossHttpResponse; var AHandled: Boolean)
    var
