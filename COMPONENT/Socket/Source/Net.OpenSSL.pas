...
begin
  ...
  if (Result = 0) then
    raise ESslInvalidLib.CreateFmt('Library Load Fail: %s', [ALibName]);
end;
...
begin
  ...
  if (Result = nil) then
    raise ESslInvalidProc.CreateFmt('GetProcAddress Fail: %s', [AProcName]);
end;
...
class procedure TSSLTools.SetCertificate(AContext: PSSL_CTX; ACertBuf: Pointer; ACertBufSize: Integer);
var
  bio_cert: PBIO;
  ssl_cert: PX509;
  store: PX509_STORE;
...
class procedure TSSLTools.SetCertificate(AContext: PSSL_CTX; const ACertStr: string);
var
  LCertBytes: TBytes;
begin
...
class procedure TSSLTools.SetCertificateFile(AContext: PSSL_CTX; const ACertFile: string);
var
  LCertBytes: TBytes;
begin
...
class procedure TSSLTools.SetPrivateKey(AContext: PSSL_CTX; APKeyBuf: Pointer; APKeyBufSize: Integer);
var
  bio_pkey: PBIO;
  ssl_pkey: PEVP_PKEY;
begin
  ...
  if (ssl_pkey = nil) then
    raise ESsl.Create('PEM_read_bio_PrivateKey');

	if (SSL_CTX_use_PrivateKey(AContext, ssl_pkey) <= 0) then
    raise ESsl.Create('SSL_CTX_use_PrivateKey');

	EVP_PKEY_free(ssl_pkey);
	BIO_free(bio_pkey);

  if (SSL_CTX_check_private_key(AContext) <= 0) then
    raise ESsl.Create('SSL_CTX_check_private_key');
end;

class procedure TSSLTools.SetPrivateKey(AContext: PSSL_CTX; const APKeyStr: string);
var
  LPKeyBytes: TBytes;
begin
...
class procedure TSSLTools.SetPrivateKeyFile(AContext: PSSL_CTX; const APKeyFile: string);
var
  LPKeyBytes: TBytes;
begin
