...
const
  LogTypeStr: array [TLogType] of string = ('INF', 'WAR', 'ERR', 'EXP');
...
begin
  ...
  for var I := Low(TLogType) to High(TLogType) do
  begin
...
begin
  _Lock(ALogType);
  try
    if (FBuffer[ALogType].Count <= 0) then
      Exit;
    ...
    try
      for var I := 0 to FBuffer[ALogType].Count - 1 do
      begin
        LLogItem := FBuffer[ALogType].Items[I];
        _WriteLogToBuffer(LLogItem);

        if not LLogItem.Time.IsSameDay(LLastTime) or (I >= FBuffer[ALogType].Count - 1) then
        begin
...
var
  LText: string;
begin
  if not (ALogType in FFilters) then
    Exit;
