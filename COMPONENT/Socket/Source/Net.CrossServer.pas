...
type
  TCrossSocketCreate = procedure(Self: Pointer; Alloc: Boolean; const AIoThreads: Integer);
  TCrossConnectionCreate = procedure(Self: Pointer; Alloc: Boolean; const AOwner: ICrossSocket; const AClientSocket: THandle; const AConnectType: TConnectType);
  TDestroy = procedure(Self: Pointer; Free: Boolean);
  TDirectSend = procedure(Self: Pointer; const ABuffer: Pointer; const ACount: Integer; const ACallback: TCrossConnectionCallback = nil);
  TTriggerConnected = procedure(Self: Pointer; const AConnection: ICrossConnection);
  TTriggerReceived = procedure(Self: Pointer; const AConnection: ICrossConnection; const ABuf: Pointer; const ALen: Integer);

{ TCrossServerConnection }

constructor TCrossServerConnection.Create(const AOwner: ICrossSocket; const AClientSocket: THandle; const AConnectType: TConnectType);
begin
  if (AOwner as TCrossServer).FSsl then
    inherited Create(AOwner, AClientSocket, AConnectType)
  else
    TCrossConnectionCreate(@TCrossConnection.Create)(Self, False, AOwner, AClientSocket, AConnectType);
end;
...
procedure TCrossServerConnection.DirectSend(const ABuffer: Pointer; const ACount: Integer; const ACallback: TCrossConnectionCallback);
begin
  if (Owner as TCrossServer).FSsl then
...
function TCrossServer.CreateConnection(const AOwner: ICrossSocket; const AClientSocket: THandle; const AConnectType: TConnectType): ICrossConnection;
begin
...
procedure TCrossServer.TriggerReceived(const AConnection: ICrossConnection; const ABuf: Pointer; const ALen: Integer);
begin
  if FSsl then
