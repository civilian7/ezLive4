...
begin
  for var LItem in STATUS_CODES do
    if (LItem.Code = AStatusCode) then
      Exit(LItem.Text);

  Result := AStatusCode.ToString;
end;

class function TCrossHttpUtils.CombinePath(const APath1, APath2: string): string;
begin
  if (APath1 = '') then
    Exit(APath2);
  ...
  var LPath1Ends := APath1.Substring(APath1.Length - 1, 1);
  var LPath2Starts := APath2.Substring(0, 1);
...
begin
  var LExt := ExtractFileExt(AFileName).Substring(1);
  for var I := 0 to High(MIME_TYPES) do
...
begin
  ...
  var Day    := StrToIntDef(Copy(ADateStr, 6, 2), 0);
  var Month  := (Pos(Copy(ADateStr, 9, 3), RFC1123_StrMonth) + 2) div 3;
  var Year   := StrToIntDef(Copy(ADateStr, 13, 4), 0);
  var Hour   := StrToIntDef(Copy(ADateStr, 18, 2), 0);
  var Min    := StrToIntDef(Copy(ADateStr, 21, 2), 0);
  var Sec    := StrToIntDef(Copy(ADateStr, 24, 2), 0);
